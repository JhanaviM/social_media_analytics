...
# ── Run All Modules ───────────────────────────────────────
# Runs each module individually so one failure does not kill the rest.
# Each module is wrapped in its own try/except.
...
def run_all(case_id):
    ...
    try:
        from app.modules.sentiment import analyze_sentiment
        save_result(case_id, 'sentiment', analyze_sentiment(posts))
    ...
    try:
        from app.modules.trending import detect_trends
        save_result(case_id, 'trending', detect_trends(posts))
        results['trending'] = 'ok'
    except Exception as e:
    ...
    try:
        from app.modules.network import analyze_network
        save_result(case_id, 'network', analyze_network(posts))
        results['network'] = 'ok'
    except Exception as e:
    ...
    try:
        from app.modules.recommendation import get_recommendations
        save_result(case_id, 'recommendation', get_recommendations(posts, case.keyword))
    ...
    try:
        from app.modules.fake_news import detect_fake_news
        save_result(case_id, 'fakenews', detect_fake_news(posts))
        results['fakenews'] = 'ok'
    except Exception as e:
    ...
    try:
        from app.modules.segmentation import segment_users
        save_result(case_id, 'segmentation', segment_users(posts))
    ...
    try:
        from app.modules.visualization import build_charts
        save_result(case_id, 'visualization', build_charts(posts))
    ...
    try:
        from app.modules.ads import optimize_ads
        save_result(case_id, 'ads', optimize_ads(posts))
        results['ads'] = 'ok'
    except Exception as e:
        results['ads'] = str(e)
    ...
    try:
        from app.modules.influencer import detect_influencers
        save_result(case_id, 'influencer', detect_influencers(posts))
    ...
    try:
        from app.modules.realtime import monitor_keywords
        save_result(case_id, 'realtime', monitor_keywords(posts, case.keyword))
        results['realtime'] = 'ok'
    except Exception as e:
    ...
    try:
        from app.modules.competitor import analyze_competitors
        save_result(case_id, 'competitor', analyze_competitors(posts, case.keyword))
    ...
    try:
        from app.modules.prediction import predict_popularity
        save_result(case_id, 'prediction', predict_popularity(posts))
